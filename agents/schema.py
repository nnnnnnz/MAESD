...
from agents.logs import logger
from agents.repo_parser import DotClassInfo
from agents.system.utils.common import any_to_str, any_to_str_set, import_class
from agents.system.utils.exceptions import handle_exception
from agents.system.utils.serialize import (
...
class Documents(BaseModel):
    ...
    def to_action_output(self) -> "ActionOutput":
        ...
        """
        from agents.actions.action_bank.action_output import ActionOutput
...
class Message(BaseModel):
    ...
    def check_instruct_content(cls, ic: Any) -> BaseModel:
        if ic and isinstance(ic, dict) and "class" in ic:
            if "mapping" in ic:
                ...
                actionnode_class = import_class("ActionNode", "agents.actions.action_node")  # avoid circular import
    ...
    def check_cause_by(cls, cause_by: Any) -> str:
        return any_to_str(cause_by if cause_by else import_class("UserRequirement", "agents.actions.add_requirement"))
    ...
    def ser_instruct_content(self, ic: BaseModel) -> Union[dict, None]:
        ...
        if ic:
            ...
            ic_type = str(type(ic))
            if "<class 'agents.actions.action_node" in ic_type:
